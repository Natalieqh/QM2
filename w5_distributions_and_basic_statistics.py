...
Original file is located at
    https://colab.research.google.com/gist/Natalieqh/ebc7dedc269d6d19f375f546e4a3e4ff/w5-distributions-and-basic-statistics.ipynb

# Difference in Differences  （Highly suggest to include in the project)
...
###could be what you need for your project
...
###this library is useful for project
...
#may not work all the time, as it is quite a large amount of data, if that happens to your prject, make it a smaller size data

###On the map, it only shows vote but not population, e.g: in new york, highly condensed population but only seemed to be a very samll size of blue color box
###pay attention do this when you are doing spatial data, because the population size canot be shown , only the county area
###so sometimes the color shown on map could be misleading, as red color seem to be dominant but Biden won this election
...
#dependent variable is r_votes, the independent variable is unemployment and population, you can do only one independent variable as well.
print(model.summary())


##we can see the more populated a county is, they tend to rate democrat
##so it is the things we conclude from the regression data, back up by our visualisation map
...
##negative correlaion between unemployment and rupbulican vote share, largest in 2020
...
# The graph shows mainly positive correlation with unemployment and rublican votes. This is different from the result in our cross-sectional regression.
# The previous one is for the whole country, this one is for a narrow area, only a few counties.
...
#we are telling what is i and t in the regression formula
...
#The more windows we do, N increase, so it increase reliability of the conclusion
#however, we also need the +-1 year window, because this is the most close year and when the timeframe increases, it actually include a lot of uncertain events
#so it is not ecoough consuming to state the conclusion
#people usually do like from the table, we can see that the influence is consistent all the time
